...
class TestEmailNotifier:
    ...
    def writeConfig(enabled, packet_with_email_notification, skip_ligo_test, skip_ste, sender_email, sender_email_password, email_receivers, developer_email_receivers):
        config = {
            "enabled": enabled,
    ...
    def test_real_send(self, notice_str):
        ...
        email_receivers = [""]
        configFile = TestEmailNotifier.writeConfig(True, [150, 151, 152], False, True, sender_email, sender_email_password, email_receivers, [])
        voe = LigoDataExtractor().extract(vp.loads(notice_str))
        correlations = [{"instrument_name" : "FERMI"}]
        sent, emailMessages = EmailNotifier(configFile).sendEmails(voe, correlations)
        # check the result 
    ...
    def test_send_email(self, notice_str):
        ...
        configFile = TestEmailNotifier.writeConfig(False, [150, 151, 152], False, True, "", "", [], [])
        voe = LigoDataExtractor().extract(vp.loads(notice_str))
        correlations = []
        ...
        configFile = TestEmailNotifier.writeConfig(False, [150, 151, 152], False, False, "", "", [], [])
        voe = LigoDataExtractor().extract(vp.loads(notice_str))
        correlations = []
        sent, emailMessage = EmailNotifier(configFile).sendEmails(voe, correlations)
        assert not sent
        ...
        # skip ligo test = True
        configFile = TestEmailNotifier.writeConfig(False, [150, 151, 152], True, False, "", "", [], [])
        voe = LigoDataExtractor().extract(vp.loads(notice_str))
        voe.is_ste = 1
        correlations = []
        sent, emailMessage = EmailNotifier(configFile).sendEmails(voe, correlations)
        ...
        configFile = TestEmailNotifier.writeConfig(False, [99], False, False, "", "", [], [])
        voe = LigoDataExtractor().extract(vp.loads(notice_str))
        voe.is_ste = 1
        correlations = []
        sent, emailMessage = EmailNotifier(configFile).sendEmails(voe, correlations)
